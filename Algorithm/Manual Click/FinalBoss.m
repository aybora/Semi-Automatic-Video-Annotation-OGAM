...
foldnumb = 21; %number of the video that we are interested in
m = readtable('batch_19002.txt'); %name of the batch of detections.
...
clearvars -except LM HM foldnumb
warning off;
...
clearvars -except PRO LM HM foldnumb
...
for k=2:size(RedTxt,1)
    if((RedTxt(k,1)>(RedTxt((k-1),1)+6))&&(RedTxt(k,2)==RedTxt((k-1),2)))
...
for i = 1:TrackCount
    ...
    [evaluationResult,clickIndex] = SimulateEvaluateTrack(outputIndex,bbox,foldnumb,RedTxt);
...
finalclick = strcat('Video',num2str(foldnumb),'FinalClickText.txt');
writematrix(FinalclickIndex,finalclick);
